...
class HROTExtendedGenerationsDialog : RuleWidget() {
    private val neighbourhoodSelector: NeighbourhoodSelector?

    init {
        neighbourhoodSelector = SharedWidgets.neighbourhoodSelector
        super.add(neighbourhoodSelector, 0, 1)
        ruleFamily = HROTExtendedGenerations()
    }
    ...
    override fun updateRule(rulestring: String?) {
        ...
        if (neighbourhoodAndWeights.value0 != null) {
            (ruleFamily as HROTExtendedGenerations).neighbourhood = neighbourhoodAndWeights.value0
            if (neighbourhoodAndWeights.value1 != null) {
                (ruleFamily as HROTExtendedGenerations).weights = neighbourhoodAndWeights.value1
            }
        }
    }
}